...
class Obstaculos {
  ...
  PImage corazon, Nido, Halcon, Halconb, Arbol, Arbolizq, 
    pajaro0, Nubelarga;
  Obstaculos() {

    for (int p=0; p<Poste.length; p++) {
      Poste[p] = loadImage("Poste"+p+".png");
      Poste[p].resize(280, 630);
    }
    for (int p=0; p<nube.length; p++) {
      nube[p] = new FBox(65, 50);
    }
    for (int p=0; p<Nube.length; p++) {
  ...
  void dibujarObs() {
    ...
    piso1 = new FCircle(500);
    piso1.setGrabbable(false);
    piso1.setStatic(true);
    piso1.setFill(0, 0);
    // piso1.noStroke();
    piso1.setPosition(540, height+200);
    mundo.add(piso1);

    piso2 = new FCircle(100);
    piso2.setGrabbable(false);
    piso2.setStatic(true);
    piso2.setFill(0, 0);
    // piso1.noStroke();
    piso2.setPosition(250, height);
    mundo.add(piso2);
    
    poste1_ = new FBox(190, 80);
    poste1_.setGrabbable(false);
    poste1_.setFill(0, 0);
    poste1_.setPosition(355, 350);
